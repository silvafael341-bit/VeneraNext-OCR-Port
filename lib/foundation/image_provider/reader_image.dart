...
Future<dynamic> debugWaitForReaderImageProcessingResult(
  ...
  return _waitForReaderImageProcessingResult(image, onCancel, checkStop, cancelSignal: cancelSignal);
}
...
Future<dynamic> _waitForReaderImageProcessingResult(
  ...
  final result = await Future.any<dynamic>([
    image,
    (cancelSignal ?? BaseImageProvider.cancelSignalOf(checkStop)).then((_) => _imageProcessingCanceled),
  ]);
...
class ReaderImageProvider extends BaseImageProvider<ReaderImageProvider> {
  ...
  Future<Uint8List> load(chunkEvents, checkStop) async {
    ...
    if (imageKey.startsWith('file://')) {
      ...
      await for (final event in ImageDownloader.loadComicImage(imageKey, sourceKey, cid, eid)) {
        checkStop();
        chunkEvents.add(ImageChunkEvent(cumulativeBytesLoaded: event.currentBytes, expectedTotalBytes: event.totalBytes));
    ...
    if (imageBytes == null) throw 'Error: Empty response body.';
    ...
    if (appdata.settings['enableCustomImageProcessing']) {
      ...
      if (script.contains('function processImage')) {
        final func = JsEngine().runCode('''(() => { $script return processImage; })()''');
        if (func is JSInvokable) {
          final autoFreeFunc = JSAutoFreeFunction(func);
          final result = autoFreeFunc([imageBytes, cid, eid, page, sourceKey]);
          if (result is Uint8List) imageBytes = result;
          else if (result is Future) {
            final futureResult = await result;
            if (futureResult is Uint8List) imageBytes = futureResult;
          } else if (result is Map) {
            final image = result['image'];
            if (image is Uint8List) imageBytes = image;
            else if (image is Future) {
              final futureImage = await image;
              if (futureImage is Uint8List) imageBytes = futureImage;
            }
          }
        }
      }
    ...
    // Embedded OCR/translation is deliberately after custom JS processing,
    // matching the Reader pipeline used by Venera-SSR.
    final translated = await TranslationEmbed.translateImage(
      imageBytes: imageBytes,
      language: 'pt-BR',
    );
    if (translated != null && translated.isNotEmpty) imageBytes = translated;

    return imageBytes;
  }

  @override
  Future<ReaderImageProvider> obtainKey(ImageConfiguration configuration) => SynchronousFuture(this);
